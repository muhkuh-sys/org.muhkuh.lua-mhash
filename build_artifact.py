...
for strMatch in glob.iglob(os.path.join(strCwd, 'lua5.1-mhash-*.xml')):
    os.remove(strMatch)
...
astrMatch = glob.glob(os.path.join(strCwd, 'lua5.1-mhash-*.xml'))
if len(astrMatch) != 1:
    raise Exception('No match found for "lua5.1-mhash-*.xml".')
...
astrCMAKE_COMPILER.append('-Dlibmhash_DIR=%s' % os.path.join(strCfg_workingFolder, 'external', 'libmhash', 'install', 'cmake'))
...
for strMatch in glob.iglob(os.path.join(strCwd, 'lua5.4-mhash-*.xml')):
    os.remove(strMatch)
...
astrMatch = glob.glob(os.path.join(strCwd, 'lua5.4-mhash-*.xml'))
if len(astrMatch) != 1:
    raise Exception('No match found for "lua5.4-mhash-*.xml".')
